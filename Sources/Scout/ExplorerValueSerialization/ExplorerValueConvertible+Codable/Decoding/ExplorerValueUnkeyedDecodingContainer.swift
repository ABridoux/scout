...
extension ExplorerValueDecoder.UnkeyedContainer {
...
extension ExplorerValueDecoder.UnkeyedContainer {
    ...
    mutating func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type) throws -> KeyedDecodingContainer<NestedKey> where NestedKey: CodingKey {
        ...
        return KeyedDecodingContainer(ExplorerValueDecoder.Container<NestedKey>(value: value, codingPath: codingPath, decoder: decoder))
    }

    mutating func nestedUnkeyedContainer() throws -> UnkeyedDecodingContainer {
        let value = array[currentIndex]
        let array = try value.array.unwrapOrThrow(.typeMismatch([ExplorerValue].self, codingPath: codingPath))
        return ExplorerValueDecoder.UnkeyedContainer(array: array, codingPath: codingPath, decoder: decoder)
    }
}
